              ...
              {filtered.map((request) => {
                ...
                return (
                  <Link key={request.id} href={`/maintenance/${request.id}`} className="interactive-row block p-4">
                    <div className="flex items-start gap-3"><div className="min-w-0 flex-1"><p className="font-semibold leading-snug text-foreground">{request.title}</p><p className="mt-0.5 text-[11px] font-medium text-muted-foreground/80">{maintenanceDisplayId(request)}</p><p className="mt-1 break-words text-xs text-muted-foreground">{location?.name} &middot; {request.area}</p></div><div className="flex shrink-0 items-center gap-1.5"><PriorityBadge priority={request.priority} /><ChevronRight className="h-4 w-4 text-muted-foreground" aria-hidden="true" /></div></div>
