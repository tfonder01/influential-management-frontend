      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <div className="contents lg:col-span-2 lg:block lg:space-y-5">
          ...
          <section className="order-6 rounded-xl border border-border bg-card p-4 shadow-sm sm:p-5">
            ...
            {canEdit && <div className="mt-4 flex gap-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">{currentUser.initials}</div><div className="flex-1"><Textarea rows={3} value={commentText} onChange={(event) => setCommentText(event.target.value)} placeholder="Add a note or answer an approval question…" disabled={commentSubmitting} /><Button size="sm" className="mt-2 gap-1.5" onClick={() => void addRequestComment()} disabled={!commentText.trim() || commentSubmitting}>{commentSubmitting ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />}Add comment</Button></div></div>}
          </section>
        </div>
