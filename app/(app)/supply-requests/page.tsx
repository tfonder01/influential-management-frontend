  ...
  return <div className="space-y-6">
    ...
    <div className="rounded-xl border border-border bg-card shadow-sm">
      ...
      {showInitialLoading ? <div className="flex flex-col items-center gap-3 py-14 text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin" /><p className="text-sm">Loading supply requests…</p></div> : showInitialError ? <div className="py-14 text-center"><p className="text-sm font-medium text-foreground">Supply requests could not be loaded.</p><Button variant="outline" size="sm" className="mt-3" onClick={() => void refreshSupplyRequests()}>Retry</Button></div> : filtered.length === 0 ? <div className="py-14 text-center"><p className="text-sm font-medium">No matching requests</p><p className="mt-1 text-xs text-muted-foreground">Try clearing one or more filters.</p></div> : <>
        <div className="divide-y divide-border md:hidden">{filtered.map((request) => { const location = locations.find((item) => item.id === request.locationId); const displayId = request.requestNumber != null ? `SUP-${request.requestNumber}` : request.id; return <Link key={request.id} href={`/supply-requests/${request.id}`} className="interactive-row block p-4"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><p className="font-semibold leading-snug">{request.title}</p><p className="mt-0.5 text-[11px] font-medium text-muted-foreground/80">{displayId}</p><p className="mt-1 break-words text-xs text-muted-foreground">{location?.name}{request.area ? ` \u00b7 ${request.area}` : ""}</p></div><div className="flex shrink-0 items-center gap-1.5"><SupplyPriorityBadge priority={request.priority} /><ChevronRight className="h-4 w-4 text-muted-foreground" aria-hidden="true" /></div></div><div className="mt-3 flex flex-wrap gap-1.5"><SupplyApprovalBadge status={request.approvalStatus} /><SupplyStatusBadge status={request.fulfillmentStatus} /></div><div className="mt-3 flex items-center justify-between gap-3 text-xs text-muted-foreground"><span>{request.quantity} item(s)</span><span className="shrink-0 font-semibold text-foreground">{money.format(request.estimatedTotal)}</span></div></Link> })}</div>
